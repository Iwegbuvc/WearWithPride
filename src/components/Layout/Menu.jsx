...
const menuItems = [
  ...
  { name: "My Orders", path: "/orders" },
  {
    ...
    submenu: [
      ...
      { name: "Shorts/Trousers", path: "/category/trousers-shorts" },
      { name: "T-shirts", path: "/category/shirts" },
  ...
  return (
    <header className="fixed top-0 left-0 w-full bg-white z-50 shadow">
      <nav className="flex flex-wrap items-center justify-between px-2 sm:px-4 md:px-8 lg:px-12 py-3 gap-y-2">
        <Link to="/" className="flex items-center gap-2 md:gap-4 lg:gap-6 justify-center min-w-0 group" style={{ textDecoration: 'none' }}>
          <img
            src={logo}
            alt="Logo"
          ...
          />
          <span className="font-extrabold text-base md:text-lg lg:text-xl xl:text-2xl text-black group-hover:text-red-600 transition-colors duration-200">
            WearWithPride
          </span>
        </Link>
              ...
              <li className="relative" key={item.name}>
                ...
                {categoryOpen && (
                  <ul
                    ...
                    className="absolute left-0 mt-3 w-56 md:w-60 bg-white border border-gray-200 rounded-xl shadow-2xl p-3 flex flex-col gap-2 z-40 animate-fade-in max-h-72 overflow-y-auto"
        ...
        <div className="hidden lg:flex items-center gap-4 min-w-0">
          ...
          <div className="relative hidden md:block">
            <form
              onSubmit={e => {
                e.preventDefault();
                if (searchTerm.trim()) navigate(`/products?q=${encodeURIComponent(searchTerm.trim())}`);
              }}
            >
              <input
                type="text"
          ...
          >
                  ...
                  <li key={item.name}>
                    ...
                    {categoryOpen && (
                      <ul
                        ...
                        className="pl-4 mt-2 flex flex-col gap-2 max-h-56 overflow-y-auto"
                        style={{ overscrollBehavior: 'contain' }}
                      >
                        {item.submenu.map((sub) => (
                          <li key={sub.name}>
            ...
            <div className="flex items-center gap-4 mt-6">
              ...
              {user ? (
                ...
                <Link
                  to="/login"
                  className="focus:outline-none cursor-pointer flex items-center gap-2 px-3 py-2 rounded text-black font-bold text-lg hover:text-red-500 no-underline"
                  onClick={() => setMobileOpen(false)}
                >
                  <FaUserCircle className="text-2xl" />
                  Login
